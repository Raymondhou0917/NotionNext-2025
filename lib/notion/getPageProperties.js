...
export async function getPagePropertiesFromDataSource(page, tagOptions) {
  const properties = {
    id: page.id
  }
  ...
  for (const [name, property] of Object.entries(page?.properties || {})) {
    ...
    switch (type) {
      case 'title':
        properties[name] = property.title?.map(item => item.plain_text).join('')
        break
      case 'rich_text':
        properties[name] = property.rich_text
          ?.map(item => item.plain_text)
          .join('')
        break
      case 'select':
      case 'status':
        properties[name] = property[type]?.name ? [property[type].name] : []
        break
      case 'multi_select':
        properties[name] = property.multi_select?.map(item => item.name) || []
        break
      case 'date':
        if (property.date) {
          properties[name] = {
            start_date: property.date.start || null,
            end_date: property.date.end || null,
      ...
      case 'last_edited_time':
        properties[name] = property.last_edited_time || null
        break
      case 'files': {
        const firstFile = property.files?.[0]
        properties[name] =
          firstFile?.[firstFile?.type]?.url || firstFile?.external?.url || ''
        break
      }
      default:
        break
  ...
  if (fieldNames) {
    Object.keys(fieldNames).forEach(key => {
      if (fieldNames[key] && properties[fieldNames[key]] !== undefined) {
        properties[key] = properties[fieldNames[key]]
      }
    })
  }
  ...
  properties.publishDay = formatDate(properties.publishDate, BLOG.LANG)
  properties.lastEditedDate = new Date(page.last_edited_time)
  properties.lastEditedDay = formatDate(
  ...
  const iconUrl = page?.icon?.[page.icon.type]?.url || properties.icon || ''
  const coverUrl = page?.cover?.[page.cover.type]?.url || ''
  properties.pageIcon = iconUrl ? mapImgUrl(iconUrl, { id: page.id }) : ''
  properties.pageCover = coverUrl ? mapImgUrl(coverUrl, { id: page.id }) : ''
  properties.pageCoverThumbnail = coverUrl
    ? mapImgUrl(coverUrl, { id: page.id }, 'block')
    : ''
  properties.ext = convertToJSON(properties?.ext)
  properties.content = []
  properties.tagItems =
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  // 外链不处理
  if (isHttpLink(postProperties.slug) || isMailOrTelLink(postProperties.slug)) {
