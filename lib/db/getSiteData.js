...
import getPageProperties, {
  ...
  getPagePropertiesFromDataSource
} from '@/lib/notion/getPageProperties'
import {
  queryDataSourceAll,
  retrieveDatabase,
  retrieveDataSource
} from '@/lib/notion/officialApi'
  ...
  return await getOrSetDataWithCache(
    ...
    async (pageId, from) => {
      if (BLOG.NOTION_API_KEY) {
        try {
          return await getSiteDataByOfficialApi({ pageId, from })
        } catch (error) {
          console.warn(
            'Official Notion API failed, fallback to legacy notion-client path',
            error
          )
        }
      }
...
async function getSiteDataByOfficialApi({ pageId, from }) {
  const database = await retrieveDatabase(idToUuid(pageId))
  const dataSourceId =
    BLOG.NOTION_DATA_SOURCE_ID || database?.data_sources?.[0]?.id

  if (!dataSourceId) {
    throw new Error(`No data source found for database ${pageId}`)
  }

  const dataSource = await retrieveDataSource(dataSourceId)
  const pages = await queryDataSourceAll(dataSourceId)
  const schema = convertDataSourcePropertiesToSchema(dataSource?.properties)
  const tagSchemaOptions = getTagOptions(schema)
  const collectionData = []

  for (const page of pages) {
    const properties =
      (await getPagePropertiesFromDataSource(page, tagSchemaOptions)) || null
    if (properties) {
      collectionData.push(properties)
    }
  }
  ...
  const latestPosts = getLatestPosts({ allPages, from, latestPostCount: 6 })
  const allNavPages = getNavPages({ allPages })
  const collection = convertDatabaseToCollection(database, dataSource)

  return {
    NOTION_CONFIG,
    notice,
    siteInfo: getSiteInfo({ collection, block: null, NOTION_CONFIG }),
    allPages,
    allNavPages,
    collection,
    collectionQuery: {},
    ...
    tagOptions,
    categoryOptions,
    rawMetadata: { type: 'data_source', id: dataSourceId },
    customNav,
    customMenu,
    postCount,
...
function convertDataSourcePropertiesToSchema(properties = {}) {
  const schema = {}
  for (const [name, property] of Object.entries(properties || {})) {
    schema[property.id || name] = {
      id: property.id || name,
      name,
      type: property.type,
      options:
...
function convertDatabaseToCollection(database, dataSource) {
  const title = database?.title?.map(item => item.plain_text).join('') || ''
  const coverUrl =
    dataSource?.cover?.[dataSource.cover?.type]?.url ||
    database?.cover?.[database.cover?.type]?.url
  const iconUrl =
    dataSource?.icon?.[dataSource.icon?.type]?.url ||
    database?.icon?.[database.icon?.type]?.url

  return {
    id: dataSource?.id || database?.id || 'site-collection',
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || BLOG.SITE_TITLE || 'NotionNext BLOG'
