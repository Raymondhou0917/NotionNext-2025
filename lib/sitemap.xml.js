...
import { getPublishedRoutablePosts } from './utils/post'
/**
 * 生成站点地图
 * @param {*} param0
 */
export async function generateSitemapXml({ allPages, NOTION_CONFIG }) {
  ...
  // 只包含有效的頁面
  const routablePosts = getPublishedRoutablePosts(allPages)
  ...
  skippedPageCount = (allPages?.length || 0) - routablePosts.length
