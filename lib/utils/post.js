...
/**
 * 过滤出可路由、可索引的已发布文章，避免 SSR 渲染到空 href。
 * @param {*} post
 * @returns
 */
export function isRoutablePost(post) {
  ...
  return !/^[0-9a-f]{32}$/i.test(slug.replace(/^\//, ''))
}

export function getPublishedRoutablePosts(allPages) {
  return allPages?.filter(isRoutablePost) || []
}
...
export async function processPostData(props, from) {
  ...
  // 推荐关联文章处理
  const allPosts = getPublishedRoutablePosts(props.allPages)
